...
$load_user = 'user_id, user_name, user_ip, last_action, last_message, user_status,
 user_action, user_color, user_rank, user_access, user_roomid, user_flood, user_avatar, user_tumb, guest, first_check, join_chat, count, user_theme';
...
if (isset($_POST['content']) && isset($_POST['bold']) 
	...
	if ($_POST['content'] != null){
		...
		{	
			...
			elseif($command[0] == '/uninstall' && $user['user_rank'] > 4 && $user['user_access'] == 4){
				if($count == 2){
					$addon_name = $command[1];
						// check if addon already exist
					$findaddon = $mysqli->query("SELECT `name` FROM `addons` WHERE `name` = '$addon_name'");
			...
			elseif($command[0] == '/silent' && $user['user_rank'] >= 4 && $user['user_access'] == 4){
			...
			elseif($command[0] == '/kick' && $user['user_rank'] > 2){
				if($count != 1){
			...
			elseif ($command[0] == '/ban' && $user['user_rank'] >= 1){
					...
					if($command[1] == 'all'){
						if($user['user_rank'] > 3){
							...
							if ($global->num_rows > 0){
								while ($globalsend = $global->fetch_assoc()){
									$global_room = $globalsend['room_id'];
									$mysqli->query("INSERT INTO `chat` (post_date, post_time, user_id, post_user, post_message, post_roomid, post_color, type, avatar) VALUES ('$time', '$post_time', '$user_id', '$lang_system', '$clearmessage', $global_room, 'bold', 'system', 'default_system_tumb.png')");
								}
							}
						}
						else {
			...
			elseif ($command[0] == '/msg' && $user['user_access'] > 3 && $user['user_rank'] >= $setting['allow_private']){
				...
				if($count != 1){
					...
					if ($findmsgtarget->num_rows > 0){
						...
						if ($count > 2){
							if ($command[1] != $name){
								$displaymsg = str_replace("$cmdmsg {$command[1]}", '', $content);
			...
			elseif ($command[0] == $cmdglobal && $user['user_rank'] > 3){

				if($count != 1){		
					$content = trim(str_replace($cmdglobal, '',$content));
					$global = $mysqli->query("SELECT `room_id` FROM `rooms` WHERE `room_id` > 0 ");
					if ($global->num_rows > 0){
						while ($globalsend = $global->fetch_assoc()){
							$global_room = $globalsend['room_id'];
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if($user['user_flood'] >= 2){
				...
				$count = $user['count'] + 1;
				$mysqli->query("UPDATE `users` SET `last_action` = $time, `user_flood` = $flood_result, `user_ip` = '$user_ip', `user_status` = 1, `user_action` = 1, `last_message` = '$content', `count` = '$count' WHERE `user_name` = '$name' AND `user_status` != '4'");
