...
*/
$load_setting = 'timezone, allow_theme, default_theme, language, domain, allow_avatar, custom1, custom2, custom_count';
$load_user = 'user_name, user_theme, user_rank, user_access, custom1, custom2';
	...
	if($findtarget->num_rows > 0){
		...
		echo '<h2 class="centered_element profile_name">' . $name . '</h2>
			...
			if($user['user_rank'] >= $setting['allow_avatar']){
				echo "<div id=\"avatar\">
				<img class=\"profile_avatar fancybox\" href=\"" . $setting['domain'] . "/$avatar_path/$avatar\" src=\"$avatar_path/$avatar\"/>
				</div>";
			}
		...
		if($setting['custom_count'] > 0 && $info['custom1'] !== ''){
			echo '<h3 class="sub_color">' . $setting['custom1'] . '</h3>
				<p>' . prolink($info['custom1']) . '</p>';
		}
		if($setting['custom_count'] > 1 && $info['custom2'] !== ''){
			echo '<h3 class="sub_color">' . $setting['custom2'] . '</h3>
