<?php
	...
	if (mysqli_connect_errno() || $check_install != 1) 
	{
		...
		else
		{
			...
			$exist = $mysqli->query("SELECT * FROM `users` WHERE `user_password` = '{$cookiepass}' AND `user_name` = '{$cookieuser}' OR `old_name` = '{$cookieuser}' AND `user_password` = '{$cookiepass}' AND `old_name` != 'e'");
			if($exist->num_rows > 0){
